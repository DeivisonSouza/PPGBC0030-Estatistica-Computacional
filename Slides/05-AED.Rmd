...
.pull-right-2[
.pull-down[
**Parte 4 - AE de alguns conjuntos de dados**

[1 - Iris - Dados de flores](#iris)
...
.right[.font150[**.green[Parte 4] - Análise exploratória de alguns <br> conjuntos de dados**]]
<br>

.right[.brown[.font120[**Conjuntos de dados:** <br> Iris Flower, Peguins, Vertebral Column, Wine]]]
...
- Vamos realizar uma análise exploratória do conjunto ".brown[**Iris Flower**]". Prefira usar os pacote estudados (**readr**, **dplyr**, **ggplot2**). Inicialmente, crie um projeto (.Rproj).

.shadow3[
## .font70[.brown[**Uma sugestão de caminho...**]]
<br>
1) Carregue o conjunto ".brown[Iris Flower]", a partir de um arquivo externo.

2) Avalie a estrutura dos dados e a existência de valores ausentes.

3) Realize uma estatística descritiva para as variáveis quantitativas. (geral e por espécie).

4) Crie uma tabela de contagem por categoria.

5) Crie as seguintes visualizações:

- Gráfico de barras (ou colunas) para demonstrar 
...
- Usando a função .brown[str()] do .orange[R-base]:
...
**3 - Obtenha um resumo estatístico do conjunto de dados?**

- Usando a função .brown[summary()] do R-base:
...
- Usando a função .brown[summarise()] do Pacote **dplyr**:
...
library(dplyr)
 data |>
  summarise(across(.cols = where(is.double),
                   .fns = tibble::lst(mean, sd, max, min),
                   na.rm = TRUE,
                   .names = "{.col}.{.fn}"
                   )
            )
...
- Usando a função .brown[skim()] do Pacote **skimr**: (consulte a [skimr_vignettes](https://cran.r-project.org/web/packages/skimr/vignettes/skimr.html))
...
- Usando a função .brown[describe()] do pacote **psych**: (consulte a [Personality project](https://personality-project.org/r/))
