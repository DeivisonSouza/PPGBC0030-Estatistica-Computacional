...
3) Obtenha um resumo estatístico (geral).
...
## Iris Flower
<br>
**2 - Avalie a estrutura dos dados. (Existe alguma desconformidade?)**
...
**Experimente:** `ncol()`, `nrow()`, `dim()`, `head()`, `tail()`
]
...
- Usando a função .brown[describe()] do pacote **psych**:

.font90[
```{r, R.options = list(width = 130), collapse = FALSE}
library(psych)
describeBy(SepalLengthCm + SepalWidthCm ~ Species, data = data)
```
]
...
- Usando a função .brown[descr()] do pacote **summarytools**: (consulte a vinheta [summarytools](https://cran.r-project.org/web/packages/summarytools/vignettes/introduction.html))

.font80[
```{r, R.options = list(width = 130), collapse = FALSE}
library(summarytools)
data |> 
  descr(stats = "common", transpose = TRUE) # c("mean", "sd", "min", "max")
```
]
...
- Cada grupo (até 4 pessoas) deverá criar um Projeto (.Rproj) e desenvolver uma AED. Inicialmente, explore os pacotes estudados (**readr**, **dplyr**, **ggplot2**, etc.). Fique a vontade para explorar outras bibliotecas especializadas em AED, como os Pacotes **DataExplorer**, **GGally**, **SmartEDA**, **summarytools**.
