...
.shadow3[
### **O que é R Markdown?**
- É um pacote/biblioteca disponível na linguagem R, que permite o uso da linguagem de marcação .brown[Markdown] e Códigos (R, Python, ...), além de textos. ([Ciência de Dados em R](https://livro.curso-r.com/9-2-r-markdown.html)).
...
- **Um arquivo .red[`.Rmd`] possui 3 componentes básicos:**

.font90[
**1) YAML**: .blue[Metadados do documento], introduzidos usando a sintaxe da linguagem [YAML](https://pt.wikipedia.org/wiki/YAML). Em geral, as informações são inseridas no cabeçalho do arquivo .red[`.Rmd`] entre o par de três traços (`---`). Em geral, várias configurações podem ser realizadas, as básicas são: .blue[título, subtítulo, autor(res), data, formato de saída].
]

```{r echo=FALSE, out.width='80%', fig.align='center', fig.cap='', dpi=600}
knitr::include_graphics("fig/part6/YAML.png")
```
